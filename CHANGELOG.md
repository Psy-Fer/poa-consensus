...
represented the way abPOA/SPOA represent it — a separate *bypass edge* around the
skipped node(s), touching them only via `delete_count` — instead of traversing
through them. This fixes a class of consensus-fabrication bugs, cleans up
edge-weight semantics, and carries the breaking changes below. It also includes
several independent correctness fixes (a severe silent-collapse bug, multi-allele
contamination, partial-read coverage).

### Breaking

- **`Consensus::read_indices` from the free functions now indexes the caller's
  input `reads` slice, not the graph's internal seed-first order.** The type is
  unchanged (`Vec<usize>`); only the values differ, and only for
  `consensus_multi`/`consensus_adaptive` calls with `seed_idx != 0` (the field
  now matches its long-documented "indices into the original `reads` slice"
  contract). Callers using `seed_idx == 0` are unaffected; any caller that
  inverted the old seed-first permutation itself must drop that workaround. The
  stateful `PoaGraph::consensus_multi` is unchanged (add-order = caller order).

- **`BubbleSite.arm_read_counts` counts only reads that genuinely confirmed an
  arm (Match, or the founding Insert), not reads that merely deleted through its
  entry node.** Type unchanged (`Vec<u32>`); values can be lower where reads
  skipped an arm's start without confirming a base. Callers summing
  `arm_read_counts` as a depth proxy should account for the reduced counts.

- **`BubbleSite.is_structural` is computed with a noise-tolerant arm-span
  measurement, so more bubbles are classified structural.** Type unchanged
  (`bool`). Previously a strict walk stopped at the first fork/reconvergence, so
  ordinary per-read noise made a genuinely long structural arm measure short and
  report `false`; it now only stops at a *significant* fork, correctly reporting
  such arms `true`.

- **`AdaptiveAction` gained two variants, `AlternateSeedRetry` and
  `MajorityFrequencyRetry`.** The enum is not `#[non_exhaustive]`, so exhaustive
  downstream `match`es must add the two arms. Semantically,
  `single_support_fraction > 0.3` no longer guarantees `NoisyTighten`:
  `consensus_adaptive` now scores several candidate remedies and returns
  whichever won (`PassThrough`, `NoisyTighten`, or one of the two new variants).

- **The diagnostic edge-weight APIs report matched-only weights, no longer
  including delete-through traversals.** Affects `PoaGraph::edge_weights()`,
  `node_out_edge_info()`, `graph_topology()` (the `GraphEdgeInfo.weight` field),
  and the `weight_threshold` comparison in `bubble_arm_lengths()`. Signatures are
  unchanged. Under pure bypass, a delete-through read is a separate bypass edge
  (invisible to `Node.out_edges`) and no longer increments the out-edge weight,
  whereas ≤ 0.2.1 a `Delete` incremented the same edge as a `Match`. For any
  graph with deletions these APIs now return **lower** weights (e.g. an edge with
  7 matching + 3 delete-through reads reported 10 in 0.2.1, reports 7 now);
  recalibrate any `bubble_arm_lengths` `weight_threshold` accordingly. Delete
  traffic is still tracked via `Node.delete_count`.

### Added

- **`analysis::consensus_fit`** — empirical per-read fit scorer (mean per-read
  Insert+Delete ops against a candidate consensus, normalized by length).
- **`consensus_fit_scored`** — standalone seed-sensitivity retry: builds and
  scores several candidate consensuses (pass-1, tightened coverage, re-seeded on
  a median-length read, majority-frequency) and returns the best-fitting one.

### Changed

- **Deletions are represented as bypass edges around skipped nodes**
  (abPOA/SPOA-style); `heaviest_path` routes over them, and the old
  `credibility_penalty` heuristic (which existed only to counteract the previous
  representation's edge "laundering") is retired. Internal graph-model change; no
  public-signature change.

### Fixed

- **Severe silent single-base collapse on the default CLI config.** When a read
  needed a wider DP band than configured — including under the CLI default
  (`band_width: 50, adaptive_band: true`) — `align()`'s terminal-cell search
  could return no endpoint and fall through to a bogus fallback, silently
  yielding an empty alignment; enough such reads collapsed the consensus to a
  single base with no error or warning. `align()` now returns `BandTooNarrow` in
  that case, and a 3-pass `align_with_retry` (configured band → required band →
  unbanded) recovers the correct alignment. The most serious correctness bug in
  this release.

- **`PoaConfig::band_width = 0` is now actually unbanded.** It previously did not
  produce a full-width DP; unbanded alignment now works as documented (and is the
  correctness fallback the retry above relies on).

- **Multi-allele read-partition contamination on periodic repeats.**
  `consensus_multi` could misassign a large fraction of reads to the wrong allele
  (measured ~29% on real GAA×30/GAA×100 HiFi). Fixed by noise-tolerant
  structural-bubble span measurement plus length-aware routing of
  bridge/ambiguous reads; ground-truth read tracing confirms misassignment down
  to ~0% on the investigated seed (a residual ~5.8% on genuinely partial,
  length-ambiguous reads is documented as inherent, output unaffected).

- **Multi-allele over-split.** `consensus_multi` no longer splits one
  length-variant allele into two when deletion noise fragments its reads; a
  candidate allele must now show a clean distinguishing bubble against confirmed
  alleles, not just length separation (`multi_skew_cag20_40` back to 2 alleles).

- **Interior-filter sequence fabrication (the Known Bugs #6–#10 class).** A run
  of fixes stopped the interior filter and heaviest-path scoring from fabricating
  bases not present in any read at bubble/fork boundaries and majority-delete
  sites (RFC1 `AAAAG`, DMD `CTT`, DAB1 `ATTTTC` real-data cases); the
  Match-vs-Delete conflation behind them is resolved by the bypass representation.

- **Partial-read coverage floor.** `coverage_threshold`'s absolute-floor fallback
  was sized off the total read count, so a partial-read population (no read spans
  the whole region) collapsed toward ~half its true length; it now uses a
  per-position local-population estimate (sliding-window over path coverage).

- **Single-allele accuracy improvements** from the bypass rework: several
  periodic-repeat scenarios that previously drifted by ±1 unit now call exactly
  (`bench/validate.py` 19/20; `bench/compare_callers.py` 16/16).

- **Seed-length-sensitivity mitigation.** An auto-selected seed atypically short
  relative to the read population could make `heaviest_path` under-call a
  homogeneous repeat's length; `consensus_adaptive`/`consensus_fit_scored` now
...
- The band-retry unbanded-rebuild **memory cliff** on multi-kb non-repetitive
  input (≈2.7 GB at 10 kb) was investigated and is **accepted as a known,
  non-target, documented cost**: there is no byte-identical way to remove it (the
  rebuild is correctness-necessary; Hirschberg linear-space and sparse storage
  were both eliminated). The sub-kb STR target regime is unaffected.
- Interior-filter Axis 1 was confirmed still load-bearing under pure bypass
  (Phase 4 of the bypass rework was investigated and intentionally not applied).
